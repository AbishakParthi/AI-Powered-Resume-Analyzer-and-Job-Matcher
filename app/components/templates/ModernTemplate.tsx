...
export default function ModernTemplate({ data, customization }: TemplateProps) {
  ...
  return (
    <article
      className={`bg-white text-black box-border w-full max-w-148.75 p-2 mx-auto ${spacing}`}
    ...
    >
      <header className="border-b pb-4" style={{ borderColor: customization.themeColor }}>
        ...
        {data.header.title && <p className="text-base mt-1">{data.header.title}</p>}
        ...
        {[data.header.linkedin, data.header.portfolio].filter(Boolean).length > 0 && (
          <p className="text-sm mt-1">
            {[data.header.linkedin, data.header.portfolio].filter(Boolean).join(" | ")}
          </p>
        )}
      </header>
